...
- **Vente en magasin et en salon (point de vente Clover autonome)** : le client utilise désormais son propre appareil Clover à écran tactile pour sonner les ventes en personne, aussi bien au comptoir qu'en salon/exposition (sans avoir besoin d'un ordinateur). La plateforme se synchronise avec cet appareil dans les deux sens : chaque vente faite sur Clover met à jour la disponibilité sur le site (webhook temps réel + sondage de secours), et chaque vente en ligne met à jour le stock dans Clover pour éviter une double vente. L'enregistrement manuel d'une vente depuis l'admin reste disponible comme solution de secours
...
L'ensemble des fonctionnalités décrites dans ce document — y compris les comptes clients, le suivi de performance, le téléversement de photos et les campagnes courriel — reste inclus dans le prix forfaitaire d'origine. Une exception : la synchronisation avec l'appareil Clover autonome (achat récent du client, non prévu à la signature du projet) est un changement de portée réel, développé mais **à chiffrer séparément**, conformément à la politique déjà énoncée ci-dessous pour toute nouvelle fonctionnalité hors périmètre. Toute nouvelle fonctionnalité qui s'ajouterait à l'avenir ferait de même l'objet d'une proposition et d'un prix séparés (voir la section Investissement).
...
| 2026-07-30 | Cambio de alcance: el cliente compró un aparato Clover de pantalla táctil autónomo (mostrador + salones/expos, sin computadora) y quiere conservarlo así. Se construyó la sincronización bidireccional entre la plataforma y Clover (Clover pasa a ser la autoridad de las ventas en persona; la plataforma sigue siendo el catálogo enriquecido y el canal en línea), más una file d'import automatique (page admin dédiée) qui détecte tout nouvel article capturé sur Clover et laisse l'équipe créer sa fiche enrichie en un clic, plus un bouton de synchronisation manuelle en secours. Este trabajo está desarrollado pero **no está incluido en el precio forfaitaire ya fijado** — es una fonctionnalité nouvelle, hors du périmètre décrit à la signature, à chiffrer séparément. |
