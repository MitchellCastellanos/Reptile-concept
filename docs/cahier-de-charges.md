...
| 2026-08-04 | Corrigé un bug de disponibilité des animaux : les fiches Clover des animaux vivants de ce client sont réglées sur « Manually manage availability », où le champ « Item stock count » et le bascule « Available » sont édités indépendamment par le personnel — un animal en vente affiche couramment un stock count à 0 tout en restant réellement disponible. La synchronisation du catalogue (`syncLinkedAnimalFromItem` et `createAnimalFromCloverData`) se basait à tort sur ce compte pour marquer un animal « sold », ce qui pouvait masquer des animaux en vente sur le site. Corrigé : le statut d'un animal n'est plus jamais dérivé du stock count Clover — seule une vente réelle (webhook/sondage de commande) le marque vendu ; un nouvel article Clover est importé comme disponible par défaut. Le stock count reste utilisé tel quel pour les Produits (accessoires), où la quantité a un sens réel. |
| 2026-08-18 | Corrigé un bug de synchronisation d'inventaire signalé par le client : les Produits reliés à Clover (ex. insectes vivants — Giant Mealworm, etc.) restaient affichés en rupture de stock sur le site alors que le stock était bien présent dans Clover. Cause : Clover garde le compte de stock sur une ressource séparée (`/item_stocks`) de la fiche article elle-même, et un réapprovisionnement fait directement sur l'appareil Clover ne met pas forcément à jour le `modifiedTime` de l'article — or à la fois le webhook (événements de type "I") et le sondage de secours (`listModifiedCloverItems`, filtré sur ce `modifiedTime`) ne réagissaient qu'aux modifications de l'article lui-même. Un Produit importé avant le correctif du 2026-08-02 (qui a ajouté la lecture du stock via `/item_stocks`) restait donc bloqué au stock capturé à l'import initial, indéfiniment, tant que personne ne modifiait autre chose sur cet article dans Clover. Corrigé : le sondage (`pollClover`, déclenché par le bouton "Synchroniser maintenant" et la tâche planifiée quotidienne) revérifie désormais le stock Clover actuel de tous les Produits déjà reliés à chaque passage, sans dépendre du `modifiedTime` ; le webhook traite aussi les événements de stock ("IS") comme les événements d'article ("I") en secours immédiat, à confirmer contre le compte sandbox du client. |
| 2026-09-05 | Le client a signalé que le bug du 2026-08-18 n'était toujours pas résolu en production pour certains Produits (Giant Mealworm, V-Mealworms, etc.) : stock à 0 dans l'admin du site malgré un stock réel dans Clover. Investigation via les logs Vercel : la tâche planifiée `/api/cron/clover-sync` échouait avec `Task timed out after 300 seconds` à chaque exécution depuis le 2026-08-18, plus une vingtaine d'erreurs 404 quotidiennes pour des Clover Item ID liés qui n'existent plus côté Clover. Cause réelle : le correctif du 2026-08-18 revérifiait le stock de chaque Produit relié un par un (2 requêtes Clover par Produit — `/items/{id}` puis `/item_stocks/{id}`), et avec la gamme d'insectes vivants de ce client (des dizaines de SKU par format de sac et par espèce), la boucle dépassait systématiquement le budget de 300 secondes de Vercel avant d'atteindre la fin de la liste — les mêmes Produits en fin de liste ne recevaient donc jamais leur stock à jour, jour après jour. Corrigé : `refreshLinkedProductStock` (et `fetchAllCloverItems`, qui souffrait du même défaut) utilisent désormais un seul appel groupé et paginé à `/item_stocks` (comme `/items`) au lieu d'un appel par Produit — une poignée de requêtes au total, peu importe le nombre de Produits reliés, ce qui élimine à la fois le dépassement de délai et le bruit des 404 répétés sur les articles Clover supprimés/recréés. |
